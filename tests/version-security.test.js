...
assert.equal(read('VERSION.txt').trim(), '1.2.0');
assert.match(read('VersionInfo.cs'), /AssemblyFileVersion\("1\.2\.0\.0"\)/);
assert.match(read('VersionInfo.cs'), /Current = "1\.2\.0"/);
assert.match(read('UpdaterVersionInfo.cs'), /AssemblyInformationalVersion\("1\.2\.0"\)/);
assert.match(read('README-FIRST.txt'), /^FreakShow 1\.2\.0/m);
